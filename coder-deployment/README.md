...
- **Post-deploy:** [`post-deploy.sh`](post-deploy.sh) — run as `sh /deploy/post-deploy.sh` when using root compose + Coolify base directory `.`. On Coolify Compose, run this **on the `coder` service**, not `database`. Default **`POST_DEPLOY_TEMPLATE_SOURCE=deployed_commit`**: see [docs/TEMPLATE_REGISTRATION.md](../docs/TEMPLATE_REGISTRATION.md).
