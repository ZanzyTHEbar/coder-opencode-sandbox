...
- **OpenCode**: Runs unchanged inside the container; state under `~/.local/share/opencode` and `~/.config/opencode` (on the volume). Workspaces can also provision a managed global OpenCode config under `~/.config/opencode` from a user-supplied Git URL or GitHub repo/tree/blob URL at creation time, clone one or more repos into `~/workspace`, and optionally cache/apply Linux dotfiles from a Git-backed source. Repo-local `.opencode` paths remain available for project-specific overrides. Workspaces can opt into public app sharing so `opencode attach https://<app-url>` works from local machines; treat public app URLs as workspace access secrets.
