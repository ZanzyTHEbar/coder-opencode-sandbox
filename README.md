...
4. Create a workspace from the template, optionally supply an OpenCode config URL, workspace repo URLs, and Linux dotfiles settings, then use the OpenCode app and terminal. If `~/.config/opencode` already exists outside the managed cache, the startup script leaves it in place and logs a warning instead of replacing it. See [docs/USER.md](docs/USER.md).
