...
OPENCODE_PROJECT_DISCOVERY_HOST=${OPENCODE_PROJECT_DISCOVERY_HOST:-0.0.0.0}
OPENCODE_PROJECT_DISCOVERY_PORT=${OPENCODE_PROJECT_DISCOVERY_PORT:-4097}
OPENCODE_PROJECT_DISCOVERY_ENABLED=${OPENCODE_PROJECT_DISCOVERY_ENABLED:-true}
...
PROJECT_DISCOVERY_PID=""
if [ "$OPENCODE_PROJECT_DISCOVERY_ENABLED" = "true" ]; then
  opencode-project-discovery-server >>"$LOG_DIR/project-discovery.log" 2>&1 </dev/null &
  PROJECT_DISCOVERY_PID=$!
  echo "$PROJECT_DISCOVERY_PID" > "$OPENCODE_DIR/project-discovery.pid"
fi
...
trap 'kill "$PROJECT_DISCOVERY_PID" "$OPENCODE_PID" 2>/dev/null || true; wait "$PROJECT_DISCOVERY_PID" "$OPENCODE_PID" 2>/dev/null || true' INT TERM
