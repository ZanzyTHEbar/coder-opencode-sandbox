...
| `SANDBOX_IMAGE` | Optional. Override OpenCode sandbox image for template push (default in compose: `ghcr.io/zanzythebar/coder-opencode-sandbox:latest`). Must be listed in `docker-compose.yml` `environment` so Coolify injects it. |
