...
- The template exposes workspace parameters for **OpenCode config URL**, **OpenCode config ref**, **OpenCode config subdirectory**, **Workspace repo URLs**, **Workspace bootstrap URL**, **Workspace bootstrap command**, **Workspace bootstrap timeout seconds**, **Workspace bootstrap failure policy**, **Linux dotfiles URL**, and **Linux dotfiles install command**. Users can paste Git URLs or GitHub repo/tree/blob URLs at workspace creation time; the startup script fetches OpenCode config into `~/.opencode-profile/` and exposes it through `~/.config/opencode`, clones any missing workspace repos into `~/workspace`, and can run generic bootstrap or dotfiles commands before OpenCode starts.
...
- **Workspace bootstrap command fails or times out:** Check `/tmp/coder-opencode-startup.log` and `/home/coder/.opencode/server.log`. The template runs `workspace_bootstrap_command` as `coder` from `workspace_bootstrap_url` when set, otherwise from `~/workspace`. The command receives `BOOTSTRAP_DIR`, `BOOTSTRAP_REPO_DIR`, and `WORKSPACE_DIR`. Default failure policy is `warn`, so OpenCode should still start; set **Workspace bootstrap failure policy** to `fail` only when strict startup is required.
- **Linux dotfiles install command fails:** Check `/tmp/coder-opencode-startup.log`. The template caches dotfiles under `~/.dotfiles-profile/` and runs `linux_dotfiles_install_command` from the selected directory on every startup. The command receives `DOTFILES_DIR`, `DOTFILES_REPO_DIR`, and `WORKSPACE_DIR` in the environment. Dotfiles command timeout and failure behavior use the workspace bootstrap timeout and failure-policy parameters.
