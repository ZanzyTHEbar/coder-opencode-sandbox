...
- **`mkdir: cannot create directory '/home/coder/workspace': Permission denied`:** The persistent volume is usually **root-owned** on first mount. The template startup script runs **`sudo chown -R coder:coder /home/coder`** before **`mkdir`**. If you still see this, the image may lack **`sudo`** / **`NOPASSWD`** (see [image/Dockerfile](../image/Dockerfile)) or the workspace was created with a **stale template** — update the workspace to the latest template version and rebuild.
