...
- **`mkdir: cannot create directory '/home/coder/workspace': Permission denied`:** The volume mounts as **root**. The template **prepends** a bootstrap to **`init_script`** and sets **`user = "0:0"`** on the workspace container so **`chown`/`mkdir` run as real root** (no reliance on `sudo`, which can fail under Docker). **Push the template** and **update** the workspace. If problems persist, check workspace logs for init failures and confirm **`SANDBOX_IMAGE`** matches an image with user **`coder`** (UID) in `/etc/passwd`.
