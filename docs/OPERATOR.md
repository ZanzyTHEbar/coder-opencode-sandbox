...
- The template exposes workspace parameters for **OpenCode config URL**, **OpenCode config ref**, and **OpenCode config subdirectory**. Users can paste a Git URL or GitHub repo/tree/blob URL at workspace creation time; the startup script fetches it into `~/.opencode-profile/` and links `~/workspace/.opencode`.
