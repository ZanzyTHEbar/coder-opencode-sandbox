...
- The template exposes workspace parameters for **OpenCode config URL**, **OpenCode config ref**, and **OpenCode config subdirectory**. Users can paste a Git or GitHub URL at workspace creation time; the startup script fetches it into `~/.opencode-profile/` and links `~/workspace/.opencode`.
...
- **Custom OpenCode config URL fails at startup:** Check `/tmp/coder-opencode-startup.log` and `/home/coder/.opencode/server.log` in the workspace. Bad URLs, private repos without credentials, or missing subdirectories now fail the startup script instead of silently booting without the profile.
