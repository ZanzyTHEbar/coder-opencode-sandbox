...
- **`mkdir: cannot create directory '/home/coder/workspace': Permission denied`:** The volume mounts as **root**. **`startup_script` runs after the agent starts**, so chown-only-in-startup was **too late** — the template now **prepends** a bootstrap to **`coder_agent.main.init_script`** (container `command`) so **`chown`/`mkdir` run before the agent**. Requires **`/usr/bin/sudo`** with **`NOPASSWD`** ([`image/Dockerfile`](../image/Dockerfile)). **Push the template** and **update** the workspace. If **`sudo` is unavailable**, you must change the image or run the workspace container as root (not covered by default).
