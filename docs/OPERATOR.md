...
- The template exposes **OpenCode app sharing** as a workspace parameter. Default **Owner only** keeps Coder auth on the app. **Public URL attach** sets `share = "public"` so local `opencode attach https://<app-url>` works without Coder browser cookies, but it exposes an unauthenticated OpenCode endpoint for anyone who has the app URL while the workspace is running. Keep wildcard routing HTTPS-only, treat public app URLs as workspace access secrets, prefer `coder port-forward` when public attach is not needed, and restrict wildcard ingress at the edge if this deployment ever stops being single-user/trusted-network oriented.
