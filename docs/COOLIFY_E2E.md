...
3. **`mkdir: ... /home/coder/workspace: Permission denied` inside the workspace** — The home volume mounts as **root**. The template **prepends** `chown`/`mkdir` to **`init_script`** and sets the workspace container **`user = 0:0`** so bootstrap runs as **root** (no `sudo`). Push the latest template and **update** the workspace. If it persists, see [OPERATOR.md](OPERATOR.md) troubleshooting.
