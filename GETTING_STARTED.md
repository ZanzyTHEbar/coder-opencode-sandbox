...
2. Create a workspace from the **OpenCode sandbox** template. If you have a custom OpenCode config repo, paste its Git URL or GitHub repo/tree/blob URL into **OpenCode config URL**. You can also set **Workspace repo URLs** to clone repo(s) into `~/workspace`, plus **Workspace bootstrap command** and optional **Workspace bootstrap URL** to prepare the Linux environment. Use **Linux dotfiles URL** and **Linux dotfiles install command** only for dotfiles-specific setup. Start the workspace.
3. Open the **OpenCode** app and the **Terminal**; confirm the UI loads and you have a shell.
4. If you set **OpenCode config URL**, confirm `~/.config/opencode` exists and resolves to the provisioned profile. If you set **Workspace repo URLs**, confirm each repo appears under `~/workspace/<repo-name>`. If you set **Workspace bootstrap command** or **Linux dotfiles install command**, confirm expected side effects landed in the home directory or Linux environment and check `/tmp/coder-opencode-startup.log` for timeout or warning messages. If you already keep your own unmanaged `~/.config/opencode`, the startup script now leaves it untouched and logs a warning instead of replacing it.
